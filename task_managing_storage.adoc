...
+
Pour plus d'informations sur la configuration d'un volume de destination pour l'accès aux données, reportez-vous à la section http://["Guide rapide de reprise après incident de volumes ONTAP 9"^].
...
+
Pour obtenir des instructions, reportez-vous au http://["Guide de migration de volumes ONTAP 9 Express"^].
